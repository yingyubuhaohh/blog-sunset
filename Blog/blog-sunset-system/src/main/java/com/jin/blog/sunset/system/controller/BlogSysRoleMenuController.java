...
public class BlogSysRoleMenuController {
    ...
    @Autowired
    private BlogSysRoleMenuService targetService;
    ...
    public R remove(@PathVariable Long id){
        if(targetService.updateById(new BlogSysRoleMenu().setId(id).setIsDelete(true))){
            return R.ok();
        }else{
            return R.error();
        }
    ...
    public R findPage(@RequestBody PageVo pageVo) {
        QueryWrapper<BlogSysRoleMenu> queryWrapper = new QueryWrapper<>();
