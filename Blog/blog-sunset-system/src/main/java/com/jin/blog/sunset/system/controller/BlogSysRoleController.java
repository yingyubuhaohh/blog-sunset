 ...
 */
@RestController
@RequestMapping("/blogSysRole")
@Api(value = "角色权限接口",tags = {"角色权限接口"})
@CrossOrigin
public class BlogSysRoleController {

    @Autowired
    private BlogSysRoleService targetService;
    ...
    public R update(@RequestBody BlogSysRole blogSysRole) {
        if(targetService.update(blogSysRole,new QueryWrapper<BlogSysRole>().eq("id",blogSysRole.getId()))){
            return R.ok();
        }else{
            return R.error();
        }
    ...
    public R remove(@PathVariable Long id){
        if(targetService.updateById(new BlogSysRole().setId(id).setIsDelete(true))){
            return R.ok();
        }else{
            return R.error();
        }
    ...
    @PostMapping("/page")
    @ApiOperation("分页查询")
    public R findPage(@RequestBody PageVo<BlogSysRole> pageVo) {
